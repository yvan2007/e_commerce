...
from accounts.mixins import AdminRequiredMixin, VendorRequiredMixin, VendorOrAdminRequiredMixin
...
class AdminDashboardView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        top_vendors = User.objects.filter(user_type='vendeur').annotate(
        ...
        order_status_stats = list(Order.objects.values('status').annotate(
            count=Count('id')
        ).order_by('status'))
        ...
        recent_reviews = ProductReview.objects.select_related(
            'user', 'product'
        ).filter(is_approved=True).order_by('-created_at')[:10]
...
class VendorDashboardView(VendorRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        shipped_orders = vendor_orders.filter(status__in=['shipped', 'delivered']).count()
        ...
        # Commandes récentes
        recent_orders = vendor_orders.select_related('user').order_by('-created_at')[:10]
        ...
        # Note moyenne
        average_rating = products.aggregate(
        ...
        current_month_orders = vendor_orders.filter(
        ...
        total_variants = products.aggregate(
        ...
        try:
            ...
            if vendor_order_items.exists():
                ...
                if new_status in ['cancelled', 'refunded']:
                    messages.error(request, 'Seul le client peut annuler ou demander un remboursement.')
                ...
                messages.success(request, f'Statut de la commande {order.order_number} mis à jour en "{order.get_status_display()}"')
            else:
...
def admin_user_create(request):
    ...
    class AdminUserCreationForm(UserCreationForm):
        ...
        country_code = forms.ChoiceField(choices=User.COUNTRY_CHOICES, initial='+225', widget=forms.Select(attrs={'class': 'form-select'}))
        ...
        user_type = forms.ChoiceField(choices=User.USER_TYPE_CHOICES, initial='client', widget=forms.Select(attrs={'class': 'form-select'}))
        
        class Meta:
            model = User
            fields = ('username', 'email', 'first_name', 'last_name', 'password1', 'password2',
                     'phone_number', 'country_code', 'user_type', 'date_of_birth', 
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            messages.success(request, _(f'L\'utilisateur {user.username} a été créé avec succès.'))
...
def admin_user_edit(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            messages.success(request, _(f'L\'utilisateur {user.username} a été modifié avec succès.'))
            return redirect('dashboard:admin_users')
    else:
...
def admin_vendor_toggle_approval(request, user_id):
    ...
    if user.user_type != 'vendeur':
        return JsonResponse({'success': False, 'error': 'Cet utilisateur n\'est pas un vendeur'}, status=400)
    ...
    messages.success(request, _(f'Le vendeur {user.username} a été {"approuvé" if vendor_profile.is_approved else "désapprouvé"}.'))
    return JsonResponse({
        'success': True,
...
def admin_products(request):
    ...
    products = Product.objects.select_related('vendor', 'category').order_by('-created_at')
...
def admin_category_delete(request, category_id):
    ...
    if products_count > 0 or subcategories_count > 0:
        messages.error(
            request, 
            _('Impossible de supprimer cette catégorie. Elle contient {} produits et {} sous-catégories.'.format(
    ...
    category.delete()
    messages.success(request, _('Catégorie "{}" supprimée avec succès.'.format(category_name)))
    return redirect('dashboard:admin_categories')
...
def approve_review(request, review_id):
    ...
    messages.success(request, _(f'L\'avis a été {"approuvé" if review.is_approved else "désapprouvé"}.'))
    return JsonResponse({
        'success': True,
