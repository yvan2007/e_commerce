...
class ReturnRequest(models.Model):
    ...
    return_type = models.CharField(
        max_length=20, choices=RETURN_TYPES, default="refund"
    )
...
class ReturnItem(models.Model):
    ...
    return_request = models.ForeignKey(
        ReturnRequest, on_delete=models.CASCADE, related_name="return_items"
    )
...
class ReturnStatusHistory(models.Model):
    ...
    return_request = models.ForeignKey(
        ReturnRequest, on_delete=models.CASCADE, related_name="status_history"
    )
    status = models.CharField(max_length=20, choices=ReturnRequest.STATUS_CHOICES)
    notes = models.TextField(blank=True)
    changed_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="return_status_changes"
    )
...
class Refund(models.Model):
    ...
    payment_method = models.CharField(
        max_length=20, choices=PAYMENT_METHODS, default="original"
    )
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="pending")
...
class StoreCredit(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="store_credits"
    )
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, validators=[MinValueValidator(Decimal("0.01"))]
    )
