...
def review_order(request, order_id):
    ...
    if order.status != "delivered":
        messages.warning(
            request, _("Vous ne pouvez évaluer que les commandes livrées.")
        )
...
def create_product_review(request, order_item_id):
    ...
    if DeliveryProductReview.objects.filter(
        user=request.user, order_item=order_item
    ).exists():
...
def create_delivery_review(request, order_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, _("Votre avis de livraison a été publié avec succès !")
            )
            return redirect("reviews:review_order", order_id=order_id)
    else:
...
def product_reviews(request, product_id):
    ...
    if search_form.is_valid():
        ...
        if with_images:
            reviews = reviews.filter(
                Q(image_1__isnull=False)
                | Q(image_2__isnull=False)
                | Q(image_3__isnull=False)
            )
...
def mark_review_helpful(request, review_id):
    ...
    if ReviewHelpful.objects.filter(user=request.user, review=review).exists():
        return JsonResponse(
            {"error": _("Vous avez déjà voté pour cet avis.")}, status=400
        )

    # Créer le vote
    helpful_vote = ReviewHelpful.objects.create(
        user=request.user, review=review, is_helpful=True
    )
    ...
    return JsonResponse({"success": True, "helpful_count": review.is_helpful})
...
class MyReviewsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            DeliveryProductReview.objects.filter(user=self.request.user)
