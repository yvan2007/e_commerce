...
from orders.models import Order, OrderItem, Cart, CartItem, ShippingAddress, OrderStatusHistory
from .serializers import (
    ...
    ShippingAddressSerializer, DashboardStatsSerializer, VendorStatsSerializer
)
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.user_type == 'vendeur':
            ...
            raise permissions.PermissionDenied("Seuls les vendeurs peuvent créer des produits")
    
    def perform_update(self, serializer):
        """Mettre à jour un produit"""
        if (self.request.user.user_type == 'vendeur' and 
            serializer.instance.vendor == self.request.user) or self.request.user.is_staff:
            serializer.save()
        else:
    ...
    def perform_destroy(self, instance):
        ...
        if (self.request.user.user_type == 'vendeur' and 
            instance.vendor == self.request.user) or self.request.user.is_staff:
            instance.delete()
        else:
            raise permissions.PermissionDenied("Vous ne pouvez pas supprimer ce produit")
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        top_products = Product.objects.filter(status='published').order_by('-sales_count')[:10]
        
        # Vendeurs les plus actifs
        top_vendors = User.objects.filter(user_type='vendeur').order_by('-vendorprofile__total_sales')[:10]
...
class VendorStatsView(APIView):
    ...
    def get(self, request):
        ...
        active_products = Product.objects.filter(vendor=request.user, status='published').count()
        ...
        reviews = ProductReview.objects.filter(product__vendor=request.user, approved=True)
