...
class Popup(models.Model):
    ...
    trigger_type = models.CharField(max_length=20, choices=TRIGGER_TYPES, default='delay')
    trigger_delay = models.IntegerField(default=3, validators=[MinValueValidator(0)])  # secondes
    trigger_scroll = models.IntegerField(default=50, validators=[MinValueValidator(0), MaxValueValidator(100)])  # pourcentage
    trigger_time = models.IntegerField(default=30, validators=[MinValueValidator(0)])  # secondes
    trigger_page_views = models.IntegerField(default=3, validators=[MinValueValidator(1)])
...
class PopupInteraction(models.Model):
    ...
    popup = models.ForeignKey(Popup, on_delete=models.CASCADE, related_name='interactions')
...
class CaptchaChallenge(models.Model):
    ...
    options = models.JSONField(default=list, blank=True)  # Pour les questions à choix multiples
...
class CaptchaSession(models.Model):
    ...
    def __str__(self):
        return f"Session {self.session_key} - {'Résolu' if self.is_solved else 'En cours'}"
...
class UserConsent(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='consents', null=True, blank=True)
    session_key = models.CharField(max_length=40, blank=True)
    ...
    def __str__(self):
        identifier = self.user.get_display_name() if self.user else f"Session {self.session_key[:8]}"
