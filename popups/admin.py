...
from .models import (
    Popup, PopupInteraction, CaptchaChallenge, CaptchaSession,
    CookieConsent, UserConsent, PrivacyPolicy, TermsOfService
)
...
class PopupAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'popup_type', 'trigger_type', 'is_active', 
        'show_count', 'conversion_count', 'conversion_rate', 'created_at'
    ]
    list_filter = [
        'popup_type', 'trigger_type', 'is_active', 'show_to_authenticated', 
        'show_to_anonymous', 'created_at'
    ]
    ...
    fieldsets = (
        ('Informations générales', {
            'fields': ('name', 'popup_type', 'title', 'content', 'button_text', 'button_url')
        }),
        ('Configuration d\'affichage', {
            'fields': (
                'trigger_type', 'trigger_delay', 'trigger_scroll', 'trigger_time', 
                'trigger_page_views'
            )
        }),
        ('Ciblage', {
        ...
        ('Statut et dates', {
            'fields': (
                'is_active', 'start_date', 'end_date', 'show_count', 'conversion_count'
            )
        }),
        ('Métadonnées', {
...
class PopupInteractionAdmin(admin.ModelAdmin):
    list_display = [
        'popup', 'user', 'action', 'ip_address', 'timestamp'
    ]
...
class CaptchaChallengeAdmin(admin.ModelAdmin):
    list_display = [
        'challenge_type', 'question', 'difficulty', 'is_active', 'created_at'
    ]
    ...
    fieldsets = (
        ('Défi', {
            'fields': ('challenge_type', 'question', 'answer', 'options')
        }),
        ('Configuration', {
...
class CaptchaSessionAdmin(admin.ModelAdmin):
    list_display = [
        'session_key', 'challenge', 'is_solved', 'attempts', 'created_at', 'expires_at'
    ]
...
class CookieConsentAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'consent_type', 'is_required', 'is_active', 'cookie_duration'
    ]
    ...
    fieldsets = (
        ('Informations', {
            'fields': ('name', 'description', 'consent_type')
        }),
        ('Configuration', {
            'fields': ('is_required', 'is_active')
        }),
        ('Cookie', {
            'fields': ('cookie_name', 'cookie_domain', 'cookie_duration')
        })
    )
...
class UserConsentAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'session_key', 'consent_summary', 'consent_date'
    ]
    list_filter = [
        'necessary_cookies', 'analytics_cookies', 'marketing_cookies', 
        'preferences_cookies', 'social_cookies', 'consent_date'
    ]
...
class PrivacyPolicyAdmin(admin.ModelAdmin):
    list_display = [
        'title', 'version', 'language', 'version_type', 'effective_date'
    ]
    ...
    fieldsets = (
        ('Informations', {
            'fields': ('title', 'version', 'version_type', 'language')
        }),
        ('Contenu', {
            'fields': ('content',)
        }),
