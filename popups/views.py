...
def generate_captcha(request):
    ...
    try:
        ...
        challenge = CaptchaChallenge.objects.filter(is_active=True).order_by('?').first()
        
        if not challenge:
            return JsonResponse({'status': 'error', 'message': 'Aucun défi captcha disponible'})
...
class TermsOfServiceView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        terms = TermsOfService.objects.filter(
            language=language,
            version_type='published'
        ).order_by('-effective_date').first()
...
def get_cookie_settings(request):
    ...
    try:
        cookies = CookieConsent.objects.filter(is_active=True).order_by('consent_type', 'name')
