...
class LoyaltyDashboardView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        available_rewards = LoyaltyReward.objects.filter(
            is_active=True,
            points_cost__lte=loyalty_points.points
...
def loyalty_history(request):
    ...
    history = LoyaltyPointsHistory.objects.filter(user=request.user).order_by('-created_at')
...
def loyalty_stats_api(request):
    ...
    monthly_stats = LoyaltyPointsHistory.objects.filter(
        user=request.user,
