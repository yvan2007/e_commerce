...
def setup_2fa(request):
    ...
    if request.method == 'POST':
        ...
        if result['success']:
            if method == 'totp':
                messages.success(request, _('Authentification à 2 facteurs configurée avec succès!'))
                return render(request, 'accounts/setup_2fa_success.html', {
            ...
            messages.error(request, result.get('error', _('Erreur lors de la configuration')))
...
def verify_2fa_setup(request, method):
    ...
    if request.method == 'GET' and method in ['email', 'sms']:
        try:
            ...
            if not result.get('success'):
                messages.warning(request, _('Erreur lors de l\'envoi du code. Veuillez réessayer.'))
    ...
    return render(request, 'accounts/verify_2fa_setup.html', {
        'method': method,
        'user': request.user
    })
...
def verify_2fa_login(request):
    ...
    try:
        ...
        if TwoFactorService.verify_2fa(user, code, method):
            ...
            return JsonResponse({'success': False, 'error': 'Code de vérification incorrect'})
...
def two_factor_required(request):
        ...
        if TwoFactorService.verify_2fa(user, code, method):
            ...
            if user.user_type == 'vendeur':
                if hasattr(user, 'vendor_profile') and user.vendor_profile.is_approved:
                    messages.success(request, f'Bienvenue dans votre espace vendeur, {user.first_name}!')
                else:
                    messages.info(request, f'Bienvenue {user.first_name}! Votre compte vendeur est en attente d\'approbation.')
            elif user.user_type == 'client':
                messages.success(request, f'Bienvenue {user.first_name}! Découvrez nos produits.')
            elif user.is_superuser:
                messages.success(request, f'Bienvenue dans l\'administration, {user.first_name}!')
    ...
    return render(request, 'accounts/two_factor_required.html', {
        'user': user
    })
