        ...
        try:
            ...
            if instance.date_joined > timezone.now() - timedelta(minutes=5):
                try:
                    ...
                    logger.error(f"Erreur lors de l'envoi de l'email de bienvenue pour {instance.username}: {e}")
            
        except Exception as e:
            logger.error(f"Erreur lors de la création du profil/panier pour {instance.username}: {e}")
