...
class UserTypeRequiredMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            messages.warning(
                request, _("Vous devez être connecté pour accéder à cette page.")
            )
...
class AdminRequiredMixin(UserTypeRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not (request.user.is_superuser or request.user.user_type == "admin"):
            messages.error(
                request, _("Accès refusé. Cette page est réservée aux administrateurs.")
            )
            return redirect("products:home_page")
...
class StaffRequiredMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not (
            request.user.is_staff or request.user.user_type in ["vendeur", "admin"]
        ):
            messages.error(
                request, _("Accès refusé. Cette page est réservée au personnel.")
            )
            return redirect("products:home_page")
