...
class EmailNotificationService:
    ...
    def send_order_status_update(self, order, old_status, new_status):
        ...
        try:
            ...
            logger.info(f"Email de mise à jour envoyé pour la commande {order.order_number}")
            return True
    ...
    def send_delivery_confirmation(self, order):
        ...
        try:
            ...
            logger.info(f"Email de livraison envoyé pour la commande {order.order_number}")
            return True
    ...
    def send_vendor_notification(self, order, vendor):
        ...
        """
        try:
            subject = f"Nouvelle commande #{order.order_number} - Produits de votre boutique"
            ...
            logger.info(f"Notification vendeur envoyée pour la commande {order.order_number}")
            return True
            ...
            for item in order.items.all():
                if item.product.main_image:
                    try:
                        ...
                        logger.warning(f"Impossible d'attacher l'image du produit {item.product.name}: {e}")
        except Exception as e:
