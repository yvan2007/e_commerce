...
class ProductListView(ListView):
    ...
    def get_queryset(self):
        ...
        if category_id:
            try:
                ...
                # Inclure la catégorie elle-même et toutes ses sous-catégories
                ...
                # Filtrer les produits de la catégorie et de ses sous-catégories
                queryset = queryset.filter(category_id__in=subcategory_ids)
            except Category.DoesNotExist:
                # Si la catégorie n'existe pas, ignorer le filtre
                pass
    ...
    def get_context_data(self, **kwargs):
        ...
        context["categories"] = (
            Category.objects.filter(is_active=True, products__status="published")
            .distinct()
            .order_by("name")
        )
...
class ProductDetailView(DetailView):
    ...
    def get_queryset(self):
        ...
        if not self.request.user.is_authenticated:
            ...
            self.request.user.is_staff or self.request.user.user_type == "vendeur"
        ):
            queryset = queryset.filter(status="published")
        else:
    ...
    def get_object(self, queryset=None):
        ...
        if obj.status != "published":
            ...
            if not (
                user.is_staff or (user.user_type == "vendeur" and obj.vendor == user)
            ):
                from django.http import Http404
    ...
    def get_context_data(self, **kwargs):
        ...
        recent_reviews = (
            DeliveryProductReview.objects.filter(product=product, is_public=True)
            .select_related("user")
    ...
    def can_user_review(self):
        ...
        delivered_items = OrderItem.objects.filter(
            order__user=self.request.user, order__status="delivered", product=product
        )
...
class ProductCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if "save_draft" in self.request.POST:
            ...
            form.instance.scheduled_publish_at = (
                None  # Annuler la programmation si archivé
            )
            ...
            if scheduled_date:
                ...
                if scheduled_date > timezone.now():
                    form.instance.status = (
                        "draft"  # Garder en brouillon jusqu'à la date
                    )
        ...
        if product.scheduled_publish_at and product.status == "draft":
            messages.success(
                self.request,
                _(
                    f'Produit créé en brouillon. Il sera publié automatiquement le {product.scheduled_publish_at.strftime("%d/%m/%Y à %H:%M")}.'
                ),
            )
        else:
    ...
    def form_valid(self, form):
        ...
        if product.scheduled_publish_at and product.status == "draft":
            messages.success(
                self.request,
                _(
                    f'Produit mis à jour. Il sera publié automatiquement le {product.scheduled_publish_at.strftime("%d/%m/%Y à %H:%M")}.'
                ),
            )
...
class CategoryListView(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Category.objects.filter(pk__in=[p.pk for p in parent_categories])
            .annotate(
        ...
        for cat in queryset:
            cat.published_products_count = (cat.direct_products_count or 0) + (
...
def category_detail_view(request, slug):
    ...
    try:
        ...
        subcategory_ids.extend(category.children.values_list("id", flat=True))

        # Récupérer tous les produits de la catégorie et ses sous-catégories
        products = (
            Product.objects.filter(category__id__in=subcategory_ids, status="published")
        ...
        if not products.exists():
            ...
            messages.info(
                request,
                f'La catégorie "{category.name}" ne contient aucun produit pour le moment.',
            )
...
def add_to_cart(request, product_id):
    ...
    if quantity <= 0:
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            ...
            return JsonResponse(
                {"success": False, "message": _("La quantité doit être positive.")},
                status=400,
            )
    ...
    if quantity > product.stock:
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            ...
            return JsonResponse(
                {"success": False, "message": _("Quantité insuffisante en stock.")},
                status=400,
            )
...
def renew_stock(request, product_id):
    ...
    if request.method == "POST":
        ...
        if product.vendor != request.user:
            messages.error(
                request, "Vous n'avez pas la permission de modifier ce produit."
            )
