...
class Category(models.Model):
    ...
    name = models.CharField(
        max_length=100, unique=True, verbose_name=_("Nom de la catégorie")
    )
...
class Tag(models.Model):
    ...
    name = models.CharField(
        max_length=50, unique=True, verbose_name=_("Nom de l'étiquette")
    )

    slug = models.SlugField(
        max_length=50, unique=True, blank=True, verbose_name=_("Slug")
    )
...
class Product(models.Model):
    ...
    sku = models.CharField(
        max_length=100, unique=True, blank=True, verbose_name=_("SKU")
    )

    description = CKEditor5Field(verbose_name=_("Description"), config_name="extends")

    short_description = models.CharField(
        max_length=500, blank=True, null=True, verbose_name=_("Description courte")
    )

    # Informations détaillées
    features = CKEditor5Field(
        blank=True, null=True, verbose_name=_("Caractéristiques"), config_name="default"
    )

    specifications = CKEditor5Field(
        blank=True, null=True, verbose_name=_("Spécifications"), config_name="default"
    )
    ...
    tags = models.ManyToManyField(
        Tag, blank=True, related_name="products", verbose_name=_("Étiquettes")
    )
    ...
    is_on_sale = models.BooleanField(default=False, verbose_name=_("En promotion"))

    sale_start_date = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Début de la promotion")
    )

    sale_end_date = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Fin de la promotion")
    )
    ...
    min_stock = models.PositiveIntegerField(default=5, verbose_name=_("Stock minimum"))

    # Images
    main_image = models.ImageField(
        upload_to="products/", blank=True, null=True, verbose_name=_("Image principale")
    )
    ...
    is_featured = models.BooleanField(default=False, verbose_name=_("Produit vedette"))

    is_digital = models.BooleanField(default=False, verbose_name=_("Produit numérique"))

    # Métriques
    views = models.PositiveIntegerField(default=0, verbose_name=_("Vues"))

    sales_count = models.PositiveIntegerField(
        default=0, verbose_name=_("Nombre de ventes")
    )
    ...
    scheduled_publish_at = models.DateTimeField(
        ...
        help_text=_(
            "Si défini, le produit sera automatiquement publié à cette date/heure"
        ),
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            self.sku
            and Product.objects.filter(sku=self.sku).exclude(pk=self.pk).exists()
        ):
            self.sku = f"SKU-{uuid.uuid4().hex[:8].upper()}"
        ...
        if (
            ...
            and not self.scheduled_publish_at
        ):
        ...
        # Si le produit est programmé et que la date est passée, le publier automatiquement
            ...
            if (
                timezone.now() >= self.scheduled_publish_at
...
class ProductImage(models.Model):
    ...
    alt_text = models.CharField(
        max_length=200, blank=True, null=True, verbose_name=_("Texte alternatif")
    )
...
class ProductVariant(models.Model):
    ...
    name = models.CharField(max_length=100, verbose_name=_("Nom de la variante"))
...
class ProductViewHistory(models.Model):
    ...
    ip_address = models.GenericIPAddressField(
        blank=True, null=True, verbose_name=_("Adresse IP")
    )

    session_key = models.CharField(
        max_length=40, blank=True, null=True, verbose_name=_("Clé de session")
    )

    viewed_at = models.DateTimeField(auto_now_add=True, verbose_name=_("Vu le"))
