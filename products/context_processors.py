...
def categories(request):
    ...
    try:
        ...
        for cat in categories:
            total_products = (cat.direct_products_count or 0) + (cat.children_products_count or 0)
            if total_products > 0:
        ...
        return {
            'categories': categories_to_display[:15]
        }
    ...
    try:
        if request.user.is_authenticated:
            ...
            try:
                ...
                cart_total = cart.get_total_price()  # Utilise la méthode du modèle qui gère les décimales
