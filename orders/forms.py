...
class CheckoutForm(forms.Form):
    ...
    shipping_first_name = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": _("Votre prénom")}
        ),
    )
    ...
    shipping_last_name = forms.CharField(
        ...
        label=_("Nom"),
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": _("Votre nom")}
        ),
    )
    ...
    shipping_address = forms.CharField(
        ...
        label=_("Adresse"),
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": _("Votre adresse complète")}
        ),
    )
    ...
    shipping_postal_code = forms.CharField(
        ...
        label=_("Code postal"),
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": _("Code postal (optionnel)")}
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user and self.user.is_authenticated:
            ...
            if hasattr(self.user, "phone_number") and self.user.phone_number:
                ...
                self.fields[
                    "shipping_phone"
                ].initial = self.user.get_full_phone_number()
            ...
            try:
                ...
                if hasattr(self, "fields") and "shipping_postal_code" in self.fields:
                    ...
                    ].initial = default_shipping.postal_code
                if hasattr(self, "fields") and "shipping_country" in self.fields:
                    self.fields["shipping_country"].initial = (
                        default_shipping.country or "Côte d'Ivoire"
                    )
                address_found = True
            except Exception:
                pass
            ...
            if not address_found:
                try:
                    ...
                    self.fields[
                        "shipping_address"
                    ].initial = default_delivery.address_line_1
                    self.fields["shipping_city"].initial = default_delivery.city
                    if (
                        hasattr(self, "fields")
                        and "shipping_postal_code" in self.fields
                    ):
                        self.fields["shipping_postal_code"].initial = (
                            default_delivery.postal_code or ""
                        )
                    if hasattr(self, "fields") and "shipping_country" in self.fields:
                        self.fields["shipping_country"].initial = (
                            default_delivery.country or "Côte d'Ivoire"
                        )
                    address_found = True
                except Exception:
                    pass
            ...
            if not address_found:
                ...
                if hasattr(self.user, "region") and self.user.region:
                    ...
                    if (
                        "shipping_address" in self.fields
                        and not self.fields["shipping_address"].initial
                    ):
    ...
    def clean_shipping_city(self):
        ...
        if city:
            ...
            if not delivery_info["zone"]:
                raise forms.ValidationError(
                    _("Nous ne livrons pas dans cette ville pour le moment.")
                )
        return city
