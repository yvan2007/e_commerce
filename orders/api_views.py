...
from delivery_system.models import Region, City
...
def calculate_delivery_fee(request):
    ...
    try:
        ...
        if not city:
            return JsonResponse({'error': 'Ville requise pour la Côte d\'Ivoire'}, status=400)
        ...
        try:
            ...
            return JsonResponse({
                ...
                'estimated_date': delivery_info['estimated_date'].strftime('%Y-%m-%d')
            })
            ...
            return JsonResponse({
                ...
                'fee': '4500',
                'estimated_days': 5,
                'estimated_date': (datetime.now() + timedelta(days=5)).strftime('%Y-%m-%d')
            })
    except Exception as e:
...
def create_order_ajax(request):
    ...
    from .models import Order, OrderItem, Cart, OrderStatusHistory
