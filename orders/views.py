...
from .forms import CheckoutForm, OrderStatusUpdateForm, OrderSearchForm
...
class CheckoutView(LoginRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context['cart_total'] = float(cart.get_total_price())  # Convertir en float pour éviter les problèmes de formatage
            context['cart_count'] = cart.get_total_items()
    ...
    def form_valid(self, form):
        ...
        calculated_delivery_fee = form.cleaned_data.get('calculated_delivery_fee', Decimal('0.00'))
        
        # Frais de livraison et taxes
        shipping_cost = calculated_delivery_fee if calculated_delivery_fee else Decimal('0.00')
        ...
        try:
            ...
            import logging
            logging.error(f"Impossible d'envoyer l'email de confirmation pour la commande {order.order_number}: {e}")
        ...
        messages.success(
            self.request, 
            _('Votre commande a été passée avec succès! Numéro de commande: {}').format(order.order_number)
        )
...
class OrderStatusUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        return (self.request.user.is_staff or 
                (self.request.user.user_type == 'vendeur' and 
                 order.items.filter(product__vendor=self.request.user).exists()))
