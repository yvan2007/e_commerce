...
def send_order_confirmation_email_signal(sender, instance, created, **kwargs):
    ...
    if created:
        try:
            ...
            logger.info(
                f"Email de confirmation de commande envoyé pour la commande {instance.order_number}"
            )
        except Exception as e:
            logger.error(
                f"Erreur lors de l'envoi de l'email de confirmation pour la commande {instance.order_number}: {e}"
            )
        ...
        if previous_status != instance.status:
            try:
                ...
                if instance.status in status_messages:
                    logger.info(
                        f"Statut de la commande {instance.order_number} changé de {previous_status} à {instance.status}"
                    )
                    # TODO: Implémenter send_order_status_email si nécessaire

            except Exception as e:
                logger.error(
                    f"Erreur lors de l'envoi de l'email de changement de statut pour la commande {instance.order_number}: {e}"
                )
