    ...
    def is_valid(self, user=None, order_amount=None):
        ...
        if user:
            ...
            if user_usage >= self.max_uses_per_user:
                return False, "Vous avez déjà utilisé ce coupon le nombre maximum de fois"
        ...
        if order_amount and order_amount < self.min_order_amount:
            return False, f"Le montant minimum de commande est de {self.min_order_amount} FCFA"
...
class CouponUsage(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-used_at']
        unique_together = ['coupon', 'order']  # Un coupon ne peut être utilisé qu'une fois par commande
