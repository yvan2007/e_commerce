    ...
    def create_test_data(self):
        ...
        for i in range(20):
            category = Category.objects.create(
                name=f"Category {i}", description=f"Category {i} description"
            )
...
class LoadTest(TestCase):
    ...
    def test_load_search(self):
        ...
        for i in range(50):
            response = self.client.get(
                reverse("search:search"), {"q": f"Product {i % 10}"}
            )
            self.assertEqual(response.status_code, 200)
...
class CachePerformanceTest(TestCase):
    ...
    def test_cache_performance(self):
        ...
        print(
            f"Cache improvement: {((first_time - second_time) / first_time) * 100:.1f}%"
        )
