...
class ProductManagementFlowTest(TestCase):
    ...
    def test_product_update_flow(self):
        ...
        response = self.client.get(reverse('products:product_update', kwargs={'pk': product.pk}))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.post(reverse('products:product_update', kwargs={'pk': product.pk}), update_data)
        self.assertEqual(response.status_code, 302)
...
class SearchAndFilterFlowTest(TestCase):
    ...
    def test_sorting_flow(self):
        ...
        response = self.client.get(reverse('products:product_list'), {'sort': '-created_at'})
        self.assertEqual(response.status_code, 200)
...
class WishlistAndComparisonFlowTest(TestCase):
    ...
    def test_wishlist_flow(self):
        ...
        response = self.client.post(
            reverse('wishlist:remove_from_wishlist', kwargs={'product_id': self.products[0].pk}),
            HTTP_X_REQUESTED_WITH='XMLHttpRequest'
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_comparison_flow(self):
        ...
        for product in self.products[:4]:  # Maximum 4 produits
            response = self.client.post(
                reverse('comparison:add_to_comparison', kwargs={'product_id': product.pk}),
                HTTP_X_REQUESTED_WITH='XMLHttpRequest'
            )
            self.assertEqual(response.status_code, 200)
            data = json.loads(response.content)
        ...
        response = self.client.post(
            reverse('comparison:remove_from_comparison', kwargs={'product_id': self.products[0].pk}),
            HTTP_X_REQUESTED_WITH='XMLHttpRequest'
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
