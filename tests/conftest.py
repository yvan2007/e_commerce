...
from notifications.models import NotificationPreference, NotificationTemplate
...
def tag():
    ...
    return Tag.objects.create(
        name="Test Tag", description="Test tag description", slug="test-tag"
    )
...
def sample_cart_items(cart, sample_products):
    ...
    for i, product in enumerate(sample_products):
        item = CartItem.objects.create(cart=cart, product=product, quantity=i + 1)
        items.append(item)
    return items
...
def sample_order_items(order, sample_products):
    ...
    for i, product in enumerate(sample_products):
        item = OrderItem.objects.create(
            order=order, product=product, quantity=i + 1, price=product.price
        )
        items.append(item)
    return items
...
def complete_payment_flow(complete_order_flow, sample_payment_methods):
    ...
    return {**complete_order_flow, "payment_transaction": transaction}
