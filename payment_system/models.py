...
class PaymentMethod(models.Model):
    ...
    logo = models.ImageField(
        upload_to="payment_logos/", blank=True, null=True, verbose_name=_("Logo")
    )
...
class PaymentTransaction(models.Model):
    ...
    transaction_id = models.CharField(
        max_length=100, unique=True, verbose_name=_("ID Transaction")
    )
    ...
    payment_method = models.ForeignKey(
        PaymentMethod, on_delete=models.CASCADE, verbose_name=_("Méthode de paiement")
    )
    ...
    fees = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00, verbose_name=_("Frais")
    )
    total_amount = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name=_("Montant total")
    )
    ...
    completed_at = models.DateTimeField(
        blank=True, null=True, verbose_name=_("Terminé le")
    )

    # Informations de paiement
    payment_reference = models.CharField(
        max_length=200, blank=True, verbose_name=_("Référence de paiement")
    )
    external_transaction_id = models.CharField(
        max_length=200, blank=True, verbose_name=_("ID Transaction Externe")
    )
    ...
    def can_be_refunded(self):
        return self.status == "completed" and self.order.status in [
            "delivered",
            "completed",
        ]
...
class MobileMoneyAccount(models.Model):
    ...
    phone_number = models.CharField(
        max_length=20, verbose_name=_("Numéro de téléphone")
    )
...
class BankCard(models.Model):
    ...
    card_type = models.CharField(
        max_length=20, choices=CARD_TYPES, verbose_name=_("Type de carte")
    )
    last_four_digits = models.CharField(
        max_length=4, verbose_name=_("4 derniers chiffres")
    )
    ...
    cardholder_name = models.CharField(
        max_length=100, verbose_name=_("Nom du titulaire")
    )
    is_verified = models.BooleanField(default=False, verbose_name=_("Vérifié"))
    is_primary = models.BooleanField(default=False, verbose_name=_("Principal"))
    created_at = models.DateTimeField(auto_now_add=True)
...
class RefundRequest(models.Model):
    ...
    reason = models.CharField(
        max_length=20, choices=REASON_CHOICES, verbose_name=_("Raison")
    )
    ...
    processed_at = models.DateTimeField(
        blank=True, null=True, verbose_name=_("Traité le")
    )
