...
from payment_system.models import PaymentTransaction, PaymentMethod
...
class PaymentInitiationView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['payment_methods'] = PaymentMethod.objects.filter(
            is_active=True,
            payment_type__in=['mobile_money', 'wave']
        )
        return context
    ...
    def post(self, request, order_id):
        ...
        if not provider or not phone_number:
            messages.error(request, 'Veuillez sélectionner une méthode de paiement et saisir votre numéro de téléphone.')
            return redirect('payment_system:payment_initiation', order_id=order_id)
        
        # Validation du numéro de téléphone
        if not self._validate_phone_number(phone_number):
            messages.error(request, 'Numéro de téléphone invalide. Format attendu: +225XXXXXXXXX')
            return redirect('payment_system:payment_initiation', order_id=order_id)
        ...
        if payment_result['success']:
            ...
            payment_transaction = PaymentTransaction.objects.create(
                order=order,
                user=request.user,
                payment_method=PaymentMethod.objects.get(name=f'{provider.upper()} Money'),
                amount=order.total_amount,
                status='pending',
            ...
            messages.success(request, f'Paiement {provider.upper()} initié avec succès ! Vérifiez votre téléphone.')
            
            # Rediriger vers la page de suivi
            return redirect('payment_system:payment_status', transaction_id=payment_transaction.id)
        else:
            messages.error(request, f'Erreur lors de l\'initiation du paiement: {payment_result["error"]}')
            return redirect('payment_system:payment_initiation', order_id=order_id)
        ...
        if status_result['success']:
            ...
            if new_status in ['SUCCESSFUL', 'completed', 'success']:
                ...
                return JsonResponse({
                    ...
                    'redirect_url': reverse('orders:order_detail', kwargs={'slug': transaction.order.order_number})
                })
                ...
                return JsonResponse({
                    ...
                    'redirect_url': reverse('payment_system:payment_initiation', kwargs={'order_id': transaction.order.id})
                })
            else:
                return JsonResponse({
                    'success': True,
        ...
        if result['success']:
            ...
            try:
                ...
                if new_status in ['SUCCESSFUL', 'completed', 'success']:
                    ...
                    logger.info(f"Paiement confirmé via webhook {provider}: {transaction_id}")
                
            except PaymentTransaction.DoesNotExist:
                logger.warning(f"Transaction non trouvée pour le webhook {provider}: {transaction_id}")
