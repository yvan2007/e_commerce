...
from .models import (
    PaymentMethod, PaymentTransaction, MobileMoneyAccount, BankCard, RefundRequest
)
...
def initiate_wave_payment(request, payment_transaction):
    ...
    try:
        ...
        wave_config = {
            ...
            'callback_url': request.build_absolute_uri(reverse_lazy('payment_system:payment_callback')),
            'return_url': request.build_absolute_uri(reverse_lazy('payment_system:payment_success')),
        }
        
        # Simuler l'enregistrement Wave
        payment_transaction.external_transaction_id = f"WAVE_{payment_transaction.id}_{int(time.time())}"
...
def initiate_orange_money_payment(request, payment_transaction):
    ...
    try:
        ...
        payment_transaction.external_transaction_id = f"ORANGE_{payment_transaction.id}_{int(time.time())}"
        payment_transaction.save()
...
def initiate_mtn_money_payment(request, payment_transaction):
    ...
    try:
        ...
        payment_transaction.external_transaction_id = f"MTN_{payment_transaction.id}_{int(time.time())}"
        payment_transaction.save()
...
def initiate_moov_money_payment(request, payment_transaction):
    ...
    try:
        ...
        payment_transaction.external_transaction_id = f"MOOV_{payment_transaction.id}_{int(time.time())}"
        payment_transaction.save()
...
def initiate_card_payment(request, payment_transaction):
    ...
    try:
        ...
        return redirect('payment_system:payment_success', transaction_id=payment_transaction.id)
...
def handle_wave_callback(data):
    ...
    try:
        ...
        payment_transaction = PaymentTransaction.objects.get(external_transaction_id=transaction_id)
...
def payment_detail(request, transaction_id):
    ...
    payment_transaction = get_object_or_404(PaymentTransaction, id=transaction_id, user=request.user)
...
def request_refund(request, transaction_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, "Erreur lors de la soumission de la demande de remboursement.")
